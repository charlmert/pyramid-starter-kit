...
# Serving the response against a match for the form_view route configured
# to /. Renders the index.pt template
...
# Matching the defined route against POST requests which will render the
# view.pt template
...
def my_view(request):
    ...
    if 'submit' in request.POST:
        ...
        try:
            ...
            return {
                'form': formHtml,
                # Validation seems to be working just the message isn't making
                # it through: "There was a problem with your submission"
                'error_message': ' '.join(
                    [request.POST['url'],
                     "is not a valid wikipedia url"]
                ),
                'content': None,
        ...
        # Removing script tags after entities have been converted (XSS
        # Protection)
